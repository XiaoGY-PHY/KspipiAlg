...
StatusCode KspipiAlg::execute()
{
    ...
    double fKsdecayL = -1000;
    double fKsdecayLerr = -1000;
    int cutReason = -1000;
    double masscut = 9999;
    double fmasscut = 9999;
    HepLorentzVector V_P_pi;
    HepLorentzVector V_M_pi;
    HepLorentzVector V_Ks;
    HepLorentzVector fV_P_pi;
    HepLorentzVector fV_M_pi;
    HepLorentzVector fV_Ks;
    ...
    for (int i = 0; i < evtRecEvent->totalCharged(); i++)
    {
        ...
        HepVector af1 = mdcKalTrk1->getFHelix();
        HepSymMatrix Ea1 = mdcKalTrk1->getZError();
        HepSymMatrix Eaf1 = mdcKalTrk1->getFError();
        VFHelix helixip3_1(point0, a1, Ea1);
        VFHelix helixipf3_1(point0, af1, Eaf1);
        helixip3_1.pivot(IP);
        helixipf3_1.pivot(IP);
        ...
        for (int j = 0; j < evtRecEvent->totalCharged(); j++)
        {
            ...
            HepVector af2 = mdcKalTrk2->getFHelix();
            HepSymMatrix Ea2 = mdcKalTrk2->getZError();
            HepSymMatrix Eaf2 = mdcKalTrk2->getFError();
            VFHelix helixip3_2(point0, a2, Ea2);
            VFHelix helixipf3_2(point0, a2, Ea2);
            helixip3_2.pivot(IP);
            helixipf3_2.pivot(IP);
            ...
            WTrackParameter fpip(mpi, mdcKalTrk1->getFHelix(), mdcKalTrk1->getFError());
            WTrackParameter fpim(mpi, mdcKalTrk2->getFHelix(), mdcKalTrk2->getFError());
            HepVector pip_val = HepVector(7, 0);
            HepVector pim_val = HepVector(7, 0);
            HepVector fpip_val = HepVector(7, 0);
            HepVector fpim_val = HepVector(7, 0);
            pip_val = pip.w(); //啥意思？
            pim_val = pim.w();
            fpip_val = fpip.w(); //啥意思？
            fpim_val = fpim.w();
            HepLorentzVector ptrktagk0(pip_val[0] + pim_val[0], pip_val[1] + pim_val[1], pip_val[2] + pim_val[2], pip_val[3] + pim_val[3]);
            HepLorentzVector fptrktagk0(fpip_val[0] + fpim_val[0], fpip_val[1] + fpim_val[1], fpip_val[2] + fpim_val[2], fpip_val[3] + fpim_val[3]);
            
            //cout << "bbbbbbbbbbbbbbbb! :: " << NNN << endl;
            if (fabs(ptrktagk0.m() - 0.497611) > 0.2 || fabs(fptrktagk0.m() - 0.497611) > 0.2)
            {
                if (cutReason == 0)
                {
                    continue;
            ...
            VertexFit *fvtxfit = VertexFit::instance();
            fvtxfit->init();

            fvtxfit->AddTrack(0, fpip);
            fvtxfit->AddTrack(1, fpim);
            fvtxfit->AddVertex(0, fvxpar, 0, 1);
            if ((!(vtxfit->Fit(0))) || (!(fvtxfit->Fit(0))))
            {
                if (cutReason == 0)
                {
                    continue;
            ...
            fvtxfit->Swim(0);
            fvtxfit->BuildVirtualParticle(0);
            WTrackParameter fwks_Trk = fvtxfit->wVirtualTrack(0);
            double fvtx_chisq = fvtxfit->chisq(0);

            WTrackParameter fwksp = fvtxfit->wtrk(0);
            WTrackParameter fwksm = fvtxfit->wtrk(1);

            HepVector fks_pip = HepVector(7, 0);
            fks_pip = fwksp.w();
            HepLorentzVector fP_pip(fks_pip[0], fks_pip[1], fks_pip[2], fks_pip[3]);
            fP_pip.boost(-0.011, 0, 0); //到质心系

            HepVector fks_pim = HepVector(7, 0);
            fks_pim = fwksm.w();
            HepLorentzVector fP_pim(fks_pim[0], fks_pim[1], fks_pim[2], fks_pim[3]);
            fP_pim.boost(-0.011, 0, 0);
            ...
            SecondVertexFit *fsvtxfit = SecondVertexFit::instance();
            fsvtxfit->init();
            fsvtxfit->setPrimaryVertex(bs);
            fsvtxfit->AddTrack(0, fvtxfit->wVirtualTrack(0));
            fsvtxfit->setVpar(fvtxfit->vpar(0));
            if ((!svtxfit->Fit()) || (!fsvtxfit->Fit()))
            {
                if (cutReason == 0)
                {
                    continue;
            ...
            if (svtxfit->chisq() > 200. || fsvtxfit->chisq() > 200.)
            {
                if (cutReason == 0)
                {
                    continue;
            ...
            double flen = fsvtxfit->decayLength();
            double flenerr = fsvtxfit->decayLengthError();

            if ((len / lenerr) <= 2 || (flen / flenerr) <= 2)
            {
                if (cutReason == 0)
                {
                    continue;
            ...
            HepLorentzVector fpks_temp = fsvtxfit->p4par();
            ...
            if (abs(pks_temp.m() - 0.497611) < masscut && abs(fpks_temp.m() - 0.497611) < fmasscut)
            {
                masscut = abs(pks_temp.m() - 0.497611);
                fmasscut = abs(fpks_temp.m() - 0.497611);
                mKs = pks_temp.m();
                fmKs = fpks_temp.m();
                pks_temp.boost(-0.011, 0, 0);
                fpks_temp.boost(-0.011, 0, 0);
                V_Ks = pks_temp;
                fV_Ks = fpks_temp;
                V_P_pi = P_pip;
                fV_P_pi = fP_pip;
    ...
    fKsdecayL_temp0 = fKsdecayL;
    fKsdecayLerr_temp0 = fKsdecayLerr;
    ...
    fV_P_pi_temp0[0] = fV_P_pi.px();
    fV_P_pi_temp0[1] = fV_P_pi.py();
    fV_P_pi_temp0[2] = fV_P_pi.pz();
    fV_P_pi_temp0[3] = fV_P_pi.e();

    fV_M_pi_temp0[0] = fV_M_pi.px();
    fV_M_pi_temp0[1] = fV_M_pi.py();
    fV_M_pi_temp0[2] = fV_M_pi.pz();
    fV_M_pi_temp0[3] = fV_M_pi.e();

    fV_Ks_temp0[0] = fV_Ks.px();
    fV_Ks_temp0[1] = fV_Ks.py();
    fV_Ks_temp0[2] = fV_Ks.pz();
    fV_Ks_temp0[3] = fV_Ks.e();
